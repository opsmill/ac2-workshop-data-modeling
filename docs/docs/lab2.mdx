...
We're going to explore using a graph database (**Neo4j**) to show the differences between a traditional relational database.
...
After you see the **Application startup complete.** means the API and database are ready.

Let's start by quickly navigating to the [API](http://localhost:8001/docs).
